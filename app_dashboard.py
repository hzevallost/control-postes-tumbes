# -*- coding: utf-8 -*-
"""
Dashboard de Control de Postes Observados - v3.11
Mejoras principales:
...
def preparar_dataframe(df_raw: pd.DataFrame) -> pd.DataFrame:
    """Prepara los datos sin confundir textos de observaciones con la columna ESTADO.

    Prioridad para identificar ESTADO:
    1) encabezado exacto ``ESTADO`` dentro de las primeras filas;
    2) columna G (índice 6), que es la estructura oficial del proyecto;
    3) columna con mayor cantidad de valores exactos PENDIENTE/ATENDIDO/CONFORME.

    Esto evita que una observación como "pendiente de resane" haga que el dashboard
    tome por error esa columna como ESTADO y filtre todos los registros.
    """
    df = df_raw.copy()

    # Normalizador de texto para comparar encabezados/estados con seguridad.
    def _norm_serie(serie: pd.Series) -> pd.Series:
        return serie.astype(str).str.upper().str.strip()

    # Detecta encabezados conocidos antes de eliminar las filas de cabecera.
    encabezados_detectados = {}
    col_estado_idx = None

    filas_revision = min(15, len(df))
    for idx_col in range(df.shape[1]):
        muestra = _norm_serie(df.iloc[:filas_revision, idx_col])
        ...
        # IMPORTANTE: coincidencia EXACTA del encabezado, no búsqueda por contenido.
        if col_estado_idx is None and muestra.eq("ESTADO").any():
            col_estado_idx = df.columns[idx_col]

    # Si no se encontró el encabezado, la estructura oficial usa la columna G.
    if col_estado_idx is None and df.shape[1] >= 7:
        candidato = df.columns[6]
        valores = _norm_serie(df[candidato]).replace({"OK": "ATENDIDO"})
        if valores.isin(ESTADOS_BASE).any():
            col_estado_idx = candidato

    # Último recurso: escoger la columna con MÁS coincidencias exactas de estado.
    # Así un comentario aislado que contenga la palabra "pendiente" no puede ganar.
    if col_estado_idx is None:
        mejor_col = None
        mejor_conteo = 0
        for col in df.columns:
            valores = _norm_serie(df[col]).replace({"OK": "ATENDIDO"})
            conteo = int(valores.isin(ESTADOS_BASE).sum())
            if conteo > mejor_conteo:
                mejor_conteo = conteo
                mejor_col = col
        if mejor_conteo > 0:
            col_estado_idx = mejor_col

    # Conserva únicamente filas de datos con un estado válido.
    if col_estado_idx is not None:
        df[col_estado_idx] = _norm_serie(df[col_estado_idx]).replace({"OK": "ATENDIDO"})
        df = df[df[col_estado_idx].isin(ESTADOS_BASE)]

    df = df.reset_index(drop=True)
    df.index = df.index + 1

    # Nombres por posición, compatibles con la hoja actual.
    if df.shape[1] >= 7:
        base_names = [
            "ZONA",
            "N° POSTE",
...
def normalizar_coordenadas_tumbes(df: pd.DataFrame, lat_col: str, lon_col: str):
    """
    Normaliza coordenadas para el proyecto de Tumbes.

    Corrige automáticamente los errores más comunes de digitación:
    - LATITUD y LONGITUD intercambiadas (ej. -80.45 / -3.56).
    - Signos positivos cuando corresponden al hemisferio sur/oeste.

    Retorna el dataframe con columnas auxiliares _LAT_MAP, _LON_MAP,
    _COORD_ESTADO y contadores de correcciones/descartes.
    """
    out = df.copy()
    out[lat_col] = pd.to_numeric(
        out[lat_col].astype(str).str.replace(",", ".", regex=False), errors="coerce"
    )
    out[lon_col] = pd.to_numeric(
        out[lon_col].astype(str).str.replace(",", ".", regex=False), errors="coerce"
    )

    # Caja geográfica holgada para el departamento/proyecto de Tumbes.
    def en_tumbes(lat, lon):
        return (-5.0 <= lat <= -2.5) and (-82.0 <= lon <= -79.0)
    ...
    for lat, lon in zip(out[lat_col], out[lon_col]):
        if pd.isna(lat) or pd.isna(lon):
            lat_map.append(float('nan'))
            lon_map.append(float('nan'))
            estados.append("SIN COORDENADAS")
            continue

        candidatos = [
            (lat, lon, "OK"),
            (lon, lat, "LAT/LON INTERCAMBIADAS"),
            (-abs(lat), -abs(lon), "SIGNOS CORREGIDOS"),
            (-abs(lon), -abs(lat), "INTERCAMBIADAS + SIGNOS CORREGIDOS"),
        ]

        elegido = None
        for la, lo, estado in candidatos:
            if en_tumbes(la, lo):
                elegido = (la, lo, estado)
                break

        if elegido is None:
            # No se grafica una coordenada que mandaría el mapa a otro continente/oceáno.
            lat_map.append(float('nan'))
            lon_map.append(float('nan'))
            estados.append("FUERA DEL ÁREA ESPERADA")
            descartadas += 1
        else:
            la, lo, estado = elegido
            lat_map.append(la)
            lon_map.append(lo)
    ...
    out["_LAT_MAP"] = lat_map
    out["_LON_MAP"] = lon_map
    out["_COORD_ESTADO"] = estados
    return out, corregidas, descartadas
...
# Mantiene también el dataframe de postes sin foto DESPUÉS para el detalle del KPI.
indices_sin_foto_despues = []
if has_service_account_secrets() and drive_files_despues and "N° POSTE" in df_filtrado.columns:
    for idx, row in df_filtrado.iterrows():
        ...
        if not meta:
            indices_sin_foto_despues.append(idx)
else:
    if "FOTO DESPUES" in df_filtrado.columns:
        vals = df_filtrado["FOTO DESPUES"].astype(str).str.strip()
        mask_sin_foto = (vals == "") | (vals.str.lower() == "nan") | (vals.str.lower() == "none")
        indices_sin_foto_despues = list(df_filtrado.index[mask_sin_foto])

sin_foto_despues = len(indices_sin_foto_despues)
df_sin_foto_despues = (
    df_filtrado.loc[indices_sin_foto_despues].copy()
    if indices_sin_foto_despues
    else df_filtrado.iloc[0:0].copy()
)


def _columnas_resumen(data_df: pd.DataFrame) -> List[str]:
    preferidas = [
        "ZONA",
        "N° POSTE",
        "TIPO / ALTURA",
    ...
    return [c for c in preferidas if c in data_df.columns]


def _grafico_kpi(indicador: str, data_df: pd.DataFrame):
    """Construye un gráfico simple y directo para el detalle de cada KPI."""
    if data_df.empty:
        return None

    indicador = indicador.upper()

    if indicador == "OBSERVADOS" and "ESTADO" in data_df.columns:
        dg = data_df["ESTADO"].value_counts().reindex(ESTADOS_BASE, fill_value=0).reset_index()
        dg.columns = ["ESTADO", "CANTIDAD"]
        fig = px.bar(
            dg,
            x="CANTIDAD",
            y="ESTADO",
        ...
        fig.update_layout(
            title="Distribución de los registros por estado",
            showlegend=False,
            height=300,
            margin=dict(t=55, b=20, l=10, r=25),
            xaxis_title="Cantidad de postes",
            yaxis_title="",
        )
        return fig

    if indicador in {"PENDIENTES", "ATENDIDOS", "CONFORMES", "SIN FOTO DESPUÉS"}:
        if "ZONA" not in data_df.columns:
            return None
        dg = data_df.groupby("ZONA").size().reset_index(name="CANTIDAD").sort_values("CANTIDAD", ascending=True)
        if dg.empty:
            return None
        color = {
            "PENDIENTES": COLOR_ESTADOS["PENDIENTE"],
            "ATENDIDOS": COLOR_ESTADOS["ATENDIDO"],
            "CONFORMES": COLOR_ESTADOS["CONFORME"],
            "SIN FOTO DESPUÉS": "#6c757d",
        }[indicador]
        fig = go.Figure(
            go.Bar(
                x=dg["CANTIDAD"],
        ...
        titulo = {
            "PENDIENTES": "Postes pendientes por zona",
            "ATENDIDOS": "Postes atendidos por zona",
            "CONFORMES": "Postes conformes por zona",
            "SIN FOTO DESPUÉS": "Postes sin evidencia DESPUÉS por zona",
        }[indicador]
        fig.update_layout(
            title=titulo,
            height=max(300, 46 * len(dg) + 100),
            margin=dict(t=55, b=20, l=10, r=35),
            xaxis_title="Cantidad de postes",
            yaxis_title="",
            showlegend=False,
        )
    ...
    if indicador == "LEVANTAMIENTO" and {"ZONA", "ESTADO"}.issubset(data_df.columns):
        dg = data_df.groupby(["ZONA", "ESTADO"]).size().unstack(fill_value=0)
        for est in ["ATENDIDO", "CONFORME"]:
            if est not in dg.columns:
                dg[est] = 0
        dg = dg[["ATENDIDO", "CONFORME"]]
        dg["TOTAL"] = dg.sum(axis=1)
        dg = dg.sort_values("TOTAL", ascending=True)
        ...
        for est in ["ATENDIDO", "CONFORME"]:
            fig.add_trace(
                go.Bar(
                    name=est,
                    x=dg[est],
                    y=dg.index.astype(str),
                    orientation="h",
                    text=[str(int(v)) if int(v) else "" for v in dg[est]],
                    textposition="inside",
                    marker_color=COLOR_ESTADOS[est],
                )
            )
        fig.update_layout(
            title="Levantamiento ejecutado por zona",
            barmode="stack",
            height=max(300, 46 * len(dg) + 100),
            margin=dict(t=55, b=20, l=10, r=25),
            xaxis_title="Cantidad de postes",
...
@st.dialog("Detalle del indicador")
def mostrar_detalle_kpi(indicador: str):
    indicador_u = indicador.upper()

    if indicador_u == "OBSERVADOS":
        detalle = df_filtrado.copy()
        titulo = f"🔍 Observados: {len(detalle)}"
        descripcion = "Todos los registros que cumplen los filtros actuales."
    elif indicador_u == "PENDIENTES":
        detalle = df_filtrado[df_filtrado["ESTADO"] == "PENDIENTE"].copy() if "ESTADO" in df_filtrado.columns else df_filtrado.iloc[0:0].copy()
        titulo = f"⚠️ Pendientes: {len(detalle)}"
        descripcion = "Postes que aún requieren atención."
    elif indicador_u == "ATENDIDOS":
        detalle = df_filtrado[df_filtrado["ESTADO"] == "ATENDIDO"].copy() if "ESTADO" in df_filtrado.columns else df_filtrado.iloc[0:0].copy()
        titulo = f"🔧 Atendidos: {len(detalle)}"
        descripcion = "Postes cuyo levantamiento fue ejecutado y está pendiente de conformidad."
    elif indicador_u == "CONFORMES":
        detalle = df_filtrado[df_filtrado["ESTADO"] == "CONFORME"].copy() if "ESTADO" in df_filtrado.columns else df_filtrado.iloc[0:0].copy()
        titulo = f"🏆 Conformes: {len(detalle)}"
        descripcion = "Postes aceptados como conformes."
    elif indicador_u == "LEVANTAMIENTO":
        detalle = df_filtrado[df_filtrado["ESTADO"].isin(["ATENDIDO", "CONFORME"])].copy() if "ESTADO" in df_filtrado.columns else df_filtrado.iloc[0:0].copy()
        titulo = f"📈 Levantamiento: {avance:.1f}%"
        descripcion = f"{len(detalle)} de {total_postes} registros están atendidos o conformes."
    else:
        detalle = df_sin_foto_despues.copy()
        titulo = f"📷 Sin foto después: {len(detalle)}"
        descripcion = "Postes que todavía no tienen evidencia fotográfica DESPUÉS en Google Drive."

    st.markdown(f"### {titulo}")
    st.caption(descripcion)

    fig = _grafico_kpi(indicador_u, detalle)
    if fig is not None:
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})

    st.markdown("#### Tabla de registros")
    if detalle.empty:
        st.info("No hay registros para este indicador con los filtros actuales.")
        return

    cols = _columnas_resumen(detalle)
    st.dataframe(
        detalle[cols] if cols else detalle,
        use_container_width=True,
        hide_index=True,
        height=min(520, max(220, 36 * (len(detalle) + 1))),
    )


# Los indicadores ahora funcionan como botones. El detalle se abre en una ventana modal.
kcols = st.columns(6)
with kcols[0]:
    if st.button(f"🔍 Observados\n\n{total_postes}", key="kpi_observados", use_container_width=True, help="Ver tabla y gráfico de todos los registros filtrados"):
        mostrar_detalle_kpi("OBSERVADOS")
    st.caption("Registros filtrados")
with kcols[1]:
    if st.button(f"⚠️ Pendientes\n\n{pendientes}", key="kpi_pendientes", use_container_width=True, help="Ver pendientes por zona y su tabla"):
        mostrar_detalle_kpi("PENDIENTES")
    st.caption("Por atender")
with kcols[2]:
    if st.button(f"🔧 Atendidos\n\n{atendidos}", key="kpi_atendidos", use_container_width=True, help="Ver atendidos por zona y su tabla"):
        mostrar_detalle_kpi("ATENDIDOS")
    st.caption("Levantamiento ejecutado")
with kcols[3]:
    if st.button(f"🏆 Conformes\n\n{conformes}", key="kpi_conformes", use_container_width=True, help="Ver conformes por zona y su tabla"):
        mostrar_detalle_kpi("CONFORMES")
    st.caption("Aceptados")
with kcols[4]:
    if st.button(f"📈 Levantamiento\n\n{avance:.1f}%", key="kpi_levantamiento", use_container_width=True, help="Ver atendidos + conformes por zona"):
        mostrar_detalle_kpi("LEVANTAMIENTO")
    st.caption("Atendidos + conformes")
with kcols[5]:
    if st.button(f"📷 Sin foto después\n\n{sin_foto_despues}", key="kpi_sin_foto", use_container_width=True, help="Ver postes sin evidencia DESPUÉS"):
        mostrar_detalle_kpi("SIN FOTO DESPUÉS")
    st.caption("Pendiente evidencia")
...
# -----------------------------------------------------------------------------
# Criterio visual:
# - Un mismo color para cada estado en todos los gráficos.
# - Mostrar cantidades y porcentajes, evitando depender solo del tamaño de la barra.
# - Indicar el total y el % de levantamiento por zona/terreno.
# - Priorizar barras sobre gráficos circulares para facilitar la comparación.

# --- 1. AVANCE POR ZONA + ESTADO GENERAL -------------------------------------
col_g1, col_g2 = st.columns([1.35, 0.85])

with col_g1:
    if "ZONA" in df_filtrado.columns and "ESTADO" in df_filtrado.columns and len(df_filtrado):
        st.markdown('<div class="centered-subheader">🗺️ Avance por Zona</div>', unsafe_allow_html=True)
        st.caption("Cantidad por estado. A la derecha se muestra el total observado y el porcentaje con levantamiento ejecutado.")

        dz = df_filtrado.groupby(["ZONA", "ESTADO"]).size().unstack(fill_value=0)
        for estado in ESTADOS_BASE:
            if estado not in dz.columns:
                dz[estado] = 0
        dz = dz[ESTADOS_BASE].copy()
        dz["TOTAL"] = dz[ESTADOS_BASE].sum(axis=1)
        dz["LEVANTADOS"] = dz.get("ATENDIDO", 0) + dz.get("CONFORME", 0)
        dz["AVANCE_PCT"] = dz.apply(
            lambda r: (r["LEVANTADOS"] / r["TOTAL"] * 100) if r["TOTAL"] else 0,
            axis=1,
        )
        # Orden ascendente para que el mayor quede visualmente arriba en barra horizontal.
        dz = dz.sort_values("TOTAL", ascending=True)

        fig_z = go.Figure()
        for estado in ESTADOS_BASE:
            vals = dz[estado].astype(int)
            pct = [
                (v / t * 100) if t else 0
                for v, t in zip(vals, dz["TOTAL"])
            ]
            labels = [
                f"{int(v)} ({p:.0f}%)" if int(v) > 0 else ""
                for v, p in zip(vals, pct)
            ]
            fig_z.add_trace(
                go.Bar(
                    name=estado,
                    y=dz.index.astype(str),
                    x=vals,
                    orientation="h",
                    text=labels,
                    textposition="inside",
                    ...
                    marker_line=dict(color="#333333", width=0.7),
                    customdata=list(zip(dz["TOTAL"], pct)),
                    hovertemplate=(
                        "<b>%{y}</b><br>"
                        + f"{estado}: %{{x}}<br>"
                        + "Participación en la zona: %{customdata[1]:.1f}%<br>"
                        + "Total observado: %{customdata[0]}<extra></extra>"
                    ),
                )
            )

        max_total_z = max(float(dz["TOTAL"].max()), 1.0)
        for zona, row in dz.iterrows():
            fig_z.add_annotation(
                x=float(row["TOTAL"]) + max_total_z * 0.02,
                y=str(zona),
                text=f"<b>Total {int(row['TOTAL'])}</b> · Lev. {row['AVANCE_PCT']:.1f}%",
                showarrow=False,
                xanchor="left",
        ...
        fig_z.update_layout(
            barmode="stack",
            height=max(340, 78 + 54 * len(dz)),
            margin=dict(t=20, b=35, l=15, r=150),
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",
            xaxis=dict(
                title="Cantidad de postes",
                showgrid=True,
                gridcolor="#e9ecef",
                zeroline=False,
                range=[0, max_total_z * 1.32],
                dtick=5 if max_total_z >= 20 else 1,
            ),
...
with col_g2:
    if "ESTADO" in df_filtrado.columns and len(df_filtrado):
        st.markdown('<div class="centered-subheader">📌 Estado General</div>', unsafe_allow_html=True)
        st.caption("Cantidad y porcentaje sobre el total de registros filtrados.")

        ce = df_filtrado["ESTADO"].value_counts()
        estados_presentes = [e for e in ESTADOS_BASE if int(ce.get(e, 0)) > 0]
        cantidades = [int(ce.get(e, 0)) for e in estados_presentes]
        porcentajes = [(v / total_postes * 100) if total_postes else 0 for v in cantidades]
        ...
        fig_e.add_trace(
            go.Bar(
                ...
                x=cantidades,
                orientation="h",
                marker_color=[COLOR_ESTADOS[e] for e in estados_presentes],
                marker_line=dict(color="#333333", width=0.8),
                text=[f"{v} · {p:.1f}%" for v, p in zip(cantidades, porcentajes)],
                textposition="outside",
                cliponaxis=False,
        ...
        max_estado = max(cantidades) if cantidades else 1
        fig_e.update_layout(
            height=340,
            margin=dict(t=25, b=35, l=10, r=65),
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",
            xaxis=dict(
                title="Cantidad de postes",
                ...
                gridcolor="#e9ecef",
                zeroline=False,
                range=[0, max_estado * 1.22],
                dtick=5 if max_estado >= 20 else 1,
            ),
            yaxis=dict(title=None, categoryorder="array", categoryarray=list(reversed(ESTADOS_BASE))),
            showlegend=False,
        )
        st.plotly_chart(fig_e, use_container_width=True, config={"displayModeBar": False})

# --- 2. TERRENO + ALTURA -----------------------------------------------------
hay_terreno = "TERRENO" in df_filtrado.columns and "ESTADO" in df_filtrado.columns and len(df_filtrado)
hay_altura = "TIPO / ALTURA" in df_filtrado.columns and "ESTADO" in df_filtrado.columns and len(df_filtrado)

if hay_terreno or hay_altura:
    if hay_terreno and hay_altura:
        col_t, col_h = st.columns(2)
    elif hay_terreno:
        col_t, col_h = st.container(), None
    else:
        col_t, col_h = None, st.container()

    if hay_terreno and col_t is not None:
        with col_t:
            st.markdown('<div class="centered-subheader">🌍 Situación por Tipo de Terreno</div>', unsafe_allow_html=True)
            st.caption("Distribución por estado; a la derecha se indica el total y el porcentaje levantado.")

            dt = df_filtrado.groupby(["TERRENO", "ESTADO"]).size().unstack(fill_value=0)
            for estado in ESTADOS_BASE:
                if estado not in dt.columns:
                    dt[estado] = 0
            dt = dt[ESTADOS_BASE].copy()
            dt["TOTAL"] = dt[ESTADOS_BASE].sum(axis=1)
            dt["LEVANTADOS"] = dt.get("ATENDIDO", 0) + dt.get("CONFORME", 0)
            dt["AVANCE_PCT"] = dt.apply(
                lambda r: (r["LEVANTADOS"] / r["TOTAL"] * 100) if r["TOTAL"] else 0,
                axis=1,
            )
            dt = dt.sort_values("TOTAL", ascending=True)

            fig_t = go.Figure()
            for estado in ESTADOS_BASE:
                vals = dt[estado].astype(int)
                labels = [str(int(v)) if int(v) > 0 else "" for v in vals]
                fig_t.add_trace(
                    go.Bar(
                        name=estado,
                        y=dt.index.astype(str),
                        x=vals,
                        orientation="h",
                        text=labels,
                        textposition="inside",
            ...
            max_total_t = max(float(dt["TOTAL"].max()), 1.0)
            for terreno, row in dt.iterrows():
                fig_t.add_annotation(
                    x=float(row["TOTAL"]) + max_total_t * 0.02,
                    y=str(terreno),
                    text=f"<b>{int(row['TOTAL'])}</b> · Lev. {row['AVANCE_PCT']:.1f}%",
                    showarrow=False,
                    xanchor="left",
                    font=dict(size=11, color="#495057"),
                )
            ...
            fig_t.update_layout(
                barmode="stack",
                height=max(330, 85 + 55 * len(dt)),
                margin=dict(t=20, b=35, l=10, r=125),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                xaxis=dict(
                    title="Cantidad de postes",
                    ...
                    gridcolor="#e9ecef",
                    zeroline=False,
                    range=[0, max_total_t * 1.30],
                    dtick=5 if max_total_t >= 20 else 1,
                ),
                yaxis=dict(title=None, automargin=True),
                legend=dict(
                    orientation="h",
    ...
    if hay_altura and col_h is not None:
        with col_h:
            st.markdown('<div class="centered-subheader">📏 Postes por Altura</div>', unsafe_allow_html=True)
            st.caption("Cantidad de postes de 9 m, 13 m u otras alturas, separados por estado.")

            da = df_filtrado.groupby(["TIPO / ALTURA", "ESTADO"]).size().unstack(fill_value=0)
            for estado in ESTADOS_BASE:
                if estado not in da.columns:
                    da[estado] = 0
            da = da[ESTADOS_BASE].copy()
            da["TOTAL"] = da[ESTADOS_BASE].sum(axis=1)

            # Orden natural simple: intenta ordenar por el valor numérico de la altura.
            def _altura_sort(v):
                s = str(v).lower().replace("m", "").strip().replace(",", ".")
                try:
                    return float(s)
                except Exception:
                    return 9999

            da = da.loc[sorted(da.index, key=_altura_sort)]
            fig_h = go.Figure()
            for estado in ESTADOS_BASE:
                vals = da[estado].astype(int)
                fig_h.add_trace(
                    go.Bar(
                        name=estado,
                        x=da.index.astype(str),
                        y=vals,
                        text=[str(int(v)) if int(v) > 0 else "" for v in vals],
                        textposition="inside",
                        textfont=dict(color="white", size=11),
                        marker_color=COLOR_ESTADOS[estado],
                        marker_line=dict(color="#333333", width=0.7),
            ...
            for altura, row in da.iterrows():
                fig_h.add_annotation(
                    x=str(altura),
                    y=float(row["TOTAL"]) + max(float(da["TOTAL"].max()), 1) * 0.035,
                    text=f"Total {int(row['TOTAL'])}",
                    showarrow=False,
                    font=dict(size=11, color="#495057"),
                )

            max_alt = max(float(da["TOTAL"].max()), 1.0)
            fig_h.update_layout(
                barmode="stack",
                height=330,
                margin=dict(t=20, b=35, l=10, r=15),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                xaxis=dict(title="Altura del poste", showgrid=False),
                yaxis=dict(
                    title="Cantidad de postes",
                    showgrid=True,
                    gridcolor="#e9ecef",
                    zeroline=False,
                    range=[0, max_alt * 1.18],
                    dtick=5 if max_alt >= 20 else 1,
                ),
                legend=dict(
                    orientation="h",
                    yanchor="bottom",
...
# -----------------------------------------------------------------------------
# MAPA OPCIONAL - OPENSTREETMAP (FOLIUM)
# -----------------------------------------------------------------------------
lat_col, lon_col = detectar_columnas_coordenadas(df_filtrado)
if lat_col and lon_col:
    map_df, coord_corregidas, coord_descartadas = normalizar_coordenadas_tumbes(
        df_filtrado, lat_col, lon_col
    )
    map_df = map_df.dropna(subset=["_LAT_MAP", "_LON_MAP"])
    ...
    if len(map_df):
        ...
        if coord_corregidas:
            st.info(
                f"Se corrigieron automáticamente {coord_corregidas} coordenada(s) con LATITUD/LONGITUD "
                "intercambiadas o con signos invertidos. Para Tumbes, la LATITUD debe ser aprox. -3.x/-4.x "
                "y la LONGITUD aprox. -80.x."
            )
        if coord_descartadas:
            st.warning(
                f"{coord_descartadas} coordenada(s) quedaron fuera del área esperada de Tumbes y no se muestran en el mapa."
            )

        # Centro inicial calculado solo con coordenadas válidas del filtro actual.
        # Así OpenStreetMap abre directamente en los postes y nunca en 0,0 (costa de Ghana).
        centro_lat = float(map_df["_LAT_MAP"].mean())
        centro_lon = float(map_df["_LON_MAP"].mean())
        zoom_inicial = 17 if len(map_df) == 1 else 13
        ...
        # Colores equivalentes a los usados por el dashboard.
        marker_colors = {
            "PENDIENTE": "red",
            "ATENDIDO": "orange",
            "CONFORME": "green",
        ...
        for _, r in map_df.iterrows():
            lat = float(r["_LAT_MAP"])
            lon = float(r["_LON_MAP"])
            zona = str(r.get("ZONA", "")).strip()
            poste = str(r.get("N° POSTE", "")).strip()
            estado = str(r.get("ESTADO", "")).strip().upper()
            terreno = str(r.get("TERRENO", "")).strip()
            altura = str(r.get("TIPO / ALTURA", "")).strip()
            observacion = str(r.get("OBSERVACIÓN / ACCIÓN", "")).strip()

            popup_parts = []
            if poste and poste.lower() != "nan":
                popup_parts.append(f"<b>Poste:</b> {poste}")
            if zona and zona.lower() != "nan":
                popup_parts.append(f"<b>Zona:</b> {zona}")
            if estado and estado.lower() != "nan":
                popup_parts.append(f"<b>Estado:</b> {estado}")
            if terreno and terreno.lower() != "nan":
                popup_parts.append(f"<b>Terreno:</b> {terreno}")
            if altura and altura.lower() not in ("nan", "none"):
                popup_parts.append(f"<b>Altura:</b> {altura}")
            if observacion and observacion.lower() not in ("nan", "none"):
                popup_parts.append(f"<b>Observación:</b> {observacion}")
            popup_parts.append(f"<b>Coordenadas:</b> {lat:.6f}, {lon:.6f}")

            # Resumen visible al pasar el cursor sobre el marcador.
            tooltip_parts = []
            if poste and poste.lower() != "nan":
                tooltip_parts.append(f"<b>Poste:</b> {poste}")
            if zona and zona.lower() != "nan":
                tooltip_parts.append(f"<b>Zona:</b> {zona}")
            if estado and estado.lower() != "nan":
                tooltip_parts.append(f"<b>Estado:</b> {estado}")
            if terreno and terreno.lower() != "nan":
                tooltip_parts.append(f"<b>Terreno:</b> {terreno}")
            if altura and altura.lower() not in ("nan", "none"):
                tooltip_parts.append(f"<b>Altura:</b> {altura}")
            tooltip_html = "<br>".join(tooltip_parts) or "Poste"

            folium.Marker(
                location=[lat, lon],
                tooltip=folium.Tooltip(tooltip_html, sticky=True),
                popup=folium.Popup("<br>".join(popup_parts), max_width=360),
                icon=folium.Icon(
                    color=marker_colors.get(estado, "blue"),
                    icon="info-sign",
                ),
            ).add_to(fmap)

        # Con varios postes ajustamos el encuadre a todos los puntos visibles.
        if len(map_df) > 1:
            bounds = [
                [float(map_df["_LAT_MAP"].min()), float(map_df["_LON_MAP"].min())],
                [float(map_df["_LAT_MAP"].max()), float(map_df["_LON_MAP"].max())],
            ]
            fmap.fit_bounds(bounds, padding=(35, 35))

        # Render de Folium directamente dentro de Streamlit, sin depender de Plotly/Mapbox.
        components.html(fmap.get_root().render(), height=560, scrolling=False)

        if len(map_df) == 1:
            r = map_df.iloc[0]
            poste_txt = str(r.get("N° POSTE", "")).strip()
            zona_txt = str(r.get("ZONA", "")).strip()
            st.caption(
                f"Centro del mapa: {float(r['_LAT_MAP']):.6f}, {float(r['_LON_MAP']):.6f}"
                + (f" · Poste {poste_txt}" if poste_txt else "")
                + (f" · {zona_txt}" if zona_txt else "")
            )

    elif coord_descartadas:
        st.markdown('<div class="centered-subheader">📍 Ubicación de Postes</div>', unsafe_allow_html=True)
        st.error(
            "Las coordenadas ingresadas no corresponden al área de Tumbes. Revisa que LATITUD sea aprox. -3.x/-4.x "
            "y LONGITUD aprox. -80.x. Ejemplo: LATITUD -3.5662 / LONGITUD -80.4513."
        )
else:
