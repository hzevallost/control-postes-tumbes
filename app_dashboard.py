# -*- coding: utf-8 -*-
"""
Dashboard de Control de Postes Observados - v2
Mejoras principales:
...
def preparar_dataframe(df_raw: pd.DataFrame) -> pd.DataFrame:
    """Mantiene la lógica del proyecto original, pero tolera columnas extra."""
    df = df_raw.copy()
    ...
    if df.shape[1] >= 7:
        ...
        df.columns = (base_names + extras)[: df.shape[1]]

    # Si en el futuro agregas columnas identificadas explícitamente en Sheets,
    # puedes renombrar aquí EXTRA_x a LATITUD / LONGITUD / FECHA / RESPONSABLE.
    return df
...
def resolver_foto_poste(
    ...
    if explicit_ref and str(explicit_ref).strip() and str(explicit_ref).strip().lower() != "nan":
        meta, reason = resolver_foto(explicit_ref, indexed_files)
        if meta:
            return meta, f"referencia explícita ({reason})"
...
if lat_col and lon_col:
    ...
    if len(map_df):
        ...
        hover = [c for c in ["N° POSTE", "ZONA", "ESTADO", "OBSERVACIÓN / ACCIÓN"] if c in map_df.columns]
    ...
    with st.expander("📍 ¿Quieres activar el mapa de postes?"):
        st.info(
            "Agrega columnas LATITUD y LONGITUD en la fuente de datos. El dashboard las detectará automáticamente. "
            "Si tus coordenadas están hoy en columnas EXTRA_x, renómbralas en preparar_dataframe()."
        )
...
if "N° POSTE" in df.columns:
    ...
    if seleccion != "-- Seleccionar --":
        ...
        st.markdown("### 📸 Registro fotográfico automático")
        st.caption(
            f"Búsqueda automática: `{zona}/{poste}_A.*` y `{zona}/{poste}_D.*` "
            "(.jpg, .jpeg, .png u otros formatos de imagen)."
        )
...
st.dataframe(df_filtrado.style.apply(resaltar_filas, axis=1), use_container_width=True)
