...
if lat_col and lon_col:
    ...
    if len(map_df):
        ...
        hover.append("_COORD_ESTADO")

        # Centro explícito del mapa. Esto evita que Plotly calcule un centro incorrecto
        # cuando solo existe un poste con coordenadas.
        map_center = {
            "lat": float(map_df["_LAT_MAP"].mean()),
            "lon": float(map_df["_LON_MAP"].mean()),
        }

        # Mapa nativo de Streamlit. Se evita depender de px.scatter_mapbox,
        # que puede no estar disponible según la versión de Plotly instalada en Cloud.
        # st.map centra el mapa directamente en las coordenadas de los datos y evita
        # el salto inicial al punto 0,0 (costa de Ghana).
        map_view = map_df.copy()
        map_view["lat"] = map_view["_LAT_MAP"].astype(float)
        map_view["lon"] = map_view["_LON_MAP"].astype(float)

        # Color por estado cuando sea posible. st.map acepta colores CSS/hex por fila.
        if "ESTADO" in map_view.columns:
            map_view["_MAP_COLOR"] = map_view["ESTADO"].map(COLOR_ESTADOS).fillna("#3366CC")
        else:
            map_view["_MAP_COLOR"] = "#3366CC"

        map_zoom = 16 if len(map_view) == 1 else 12
        try:
            st.map(
                map_view,
                latitude="lat",
            ...
            # Compatibilidad con versiones de Streamlit donde algunos argumentos
            # opcionales de st.map todavía no están disponibles.
            st.map(
                map_view[["lat", "lon"]],
                latitude="lat",
                longitude="lon",
        ...
        # Cuando solo hay un poste, mostramos también su coordenada exacta debajo
        # para facilitar la verificación en campo.
        if len(map_view) == 1:
            r = map_view.iloc[0]
            poste_txt = str(r.get("N° POSTE", "")).strip()
            zona_txt = str(r.get("ZONA", "")).strip()
            st.caption(
                f"Centro del mapa: {float(r['lat']):.6f}, {float(r['lon']):.6f}"
